---
title: "Comparing R and Rcpp Implementations"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Comparing R and Rcpp Implementations}
...
`wearableAnomaly` ships both R and Rcpp implementations for its computationally
intensive routines. By default, the package selects the native code path when
the shared library is available, but you can switch back to pure R for parity or
teaching purposes. This vignette illustrates:

1. **PELT** vs. the reference `{changepoint}` implementation.
2. **E-divisive** vs. `{ecp}`.
3. How to toggle between R and Rcpp backends and benchmark them.

> **Note**: Comparator packages are optional. Install them beforehand to run the
...
Setting `wearableAnomaly.use_rcpp = FALSE` forces the pure R fallback, which is
useful for debugging or machines without compilers.

## PELT vs `{changepoint}`

```{r pelt-compare, eval=FALSE}
library(changepoint)
...
The Rcpp core (`src/pelt_core.cpp`) uses prefix sums and pruning to match the
reference algorithm’s accuracy while reducing allocations and loop overhead.

## E-divisive vs `{ecp}`

```{r ediv-compare, eval=FALSE}
library(ecp)
...
The Rcpp helper (`energy_stat_segment()`) precomputes pairwise distances and
reuses them during permutation tests, resulting in substantial runtime savings
while matching `{ecp}` across seeds.
...
`wearableAnomaly.use_rcpp = FALSE` to avoid warnings on machines without build
tools.

## Reproducing the tests

The parity and timing tests used for development live under
`tests/testthat/test_pelt.R` and `tests/testthat/test_edivisive.R`. Run them
directly with:
...
They automatically skip when `{changepoint}`, `{ecp}`, or the Rcpp backend are
unavailable.
